...
def load_history(out_dir: Path):
    ...
    hx = np.fromfile(out_dir / "hx_history.bin", dtype=np.float32).reshape(nt_rec, nx, ny)
    hy = np.fromfile(out_dir / "hy_history.bin", dtype=np.float32).reshape(nt_rec, nx, ny)
    ez = np.fromfile(out_dir / "ez_history.bin", dtype=np.float32).reshape(nt_rec, nx, ny)
    er = np.fromfile(out_dir / "erzz.bin", dtype=np.float64).reshape(nx, ny)
    return meta, hx, hy, ez, er
...
def animate(out_dir: Path, cfg: dict, fps: int, dynamic_clim: bool, clim_smooth: float, pad: float = 1e-12):
    meta, hx, hy, ez, er = load_history(out_dir)
    ...
    im_hx = ax_hx.imshow(hx[0].T, origin="lower", aspect="auto", extent=extent, cmap="jet")
    fig.colorbar(im_hx, ax=ax_hx).set_label("Hx")
    ax_hx.set_title("Hx")
    ax_hx.set_xlabel("x (m)")
    ax_hx.set_ylabel("y (m)")

    im_hy = ax_hy.imshow(hy[0].T, origin="lower", aspect="auto", extent=extent, cmap="jet")
    fig.colorbar(im_hy, ax=ax_hy).set_label("Hy")
    ax_hy.set_title("Hy")
    ax_hy.set_xlabel("x (m)")
    ax_hy.set_ylabel("y (m)")

    im_ez = ax_ez.imshow(ez[0].T, origin="lower", aspect="auto", extent=extent, cmap="jet")
    ...
    time_text = ax_ez.text(
        0.02,
        0.02,
        "",
        transform=ax_ez.transAxes,
        bbox=dict(facecolor="white", alpha=0.7, edgecolor="none"),
    )

    if dynamic_clim:
        smoothed_vmax = max(np.max(np.abs(hx[0])) + pad, np.max(np.abs(hy[0])) + pad, np.max(np.abs(ez[0])) + pad)
        ...
        vmax = max(np.max(np.abs(hx)) + pad, np.max(np.abs(hy)) + pad, np.max(np.abs(ez)) + pad)
        im_hx.set_clim(-vmax, vmax)
        im_hy.set_clim(-vmax, vmax)
        im_ez.set_clim(-vmax, vmax)
        smoothed_vmax = vmax

    def update(frame):
        nonlocal smoothed_vmax
        im_hx.set_data(hx[frame].T)
        im_hy.set_data(hy[frame].T)
        im_ez.set_data(ez[frame].T)

        if dynamic_clim:
            vmax_now = max(np.max(np.abs(hx[frame])) + pad, np.max(np.abs(hy[frame])) + pad, np.max(np.abs(ez[frame])) + pad)
...
def _monitor_geom_from_cfg(cfg: dict, monitor_index: int):
    ...
    if ix0 != ix1 and iy0 == iy1:
        x = np.arange(ix0, ix1, dtype=float) * dx
        y = np.full_like(x, iy0 * dy, dtype=float)
        ori = "horizontal"
        dl = dx
    elif ix0 == ix1 and iy0 != iy1:
        y = np.arange(iy0, iy1, dtype=float) * dy
        x = np.full_like(y, ix0 * dx, dtype=float)
        ori = "vertical"
        dl = dy
    else:
...
def compute_nf2ff_python(out_dir: Path, cfg: dict):
    ...
    for side, mi in sides.items():
        ...
        if nline <= 0:
            side_data[side] = None
            continue
        ez = np.fromfile(out_dir / f"monitor_time_ez_{mi}.bin", dtype=np.float64).reshape(nt, nline)
        hx = np.fromfile(out_dir / f"monitor_time_hx_{mi}.bin", dtype=np.float64).reshape(nt, nline)
        hy = np.fromfile(out_dir / f"monitor_time_hy_{mi}.bin", dtype=np.float64).reshape(nt, nline)
        ez_f = _phasor_series(ez, dt, freqs) * eta0
        hx_f = _phasor_series(hx, dt, freqs)
        hy_f = _phasor_series(hy, dt, freqs)
        _, x, y, dl = _monitor_geom_from_cfg(cfg, int(mi))
        side_data[side] = {"Ez": ez_f, "Hx": hx_f, "Hy": hy_f, "x": x, "y": y, "dl": dl}
    ...
    n_theta = -int_side("bottom", "Hx") - int_side("right", "Hy") + int_side("top", "Hx") + int_side("left", "Hy")
    l_phi = -sph * int_side("bottom", "Ez") + cph * int_side("right", "Ez") + sph * int_side("top", "Ez") - cph * int_side("left", "Ez")

    e_theta = k0 * (eta0 * n_theta + l_phi)
    h_phi = k0 * (l_phi / eta0 + n_theta)
    gmax = np.max(np.abs(gsrc))
    if gmax > 0:
        good = np.abs(gsrc) >= 1e-6 * gmax
        e_theta[good, :] /= gsrc[good, None]
        h_phi[good, :] /= gsrc[good, None]
        e_theta[~good, :] = 0.0
        h_phi[~good, :] = 0.0

    p_theta = 0.5 * np.real(e_theta * h_phi)
    with (out_dir / "nf2ff.csv").open("w", encoding="utf-8") as f:
        f.write("freq_hz,phi_rad,e_theta_re,e_theta_im,h_phi_re,h_phi_im,p_theta_re\n")
        for fi, fr in enumerate(freqs):
            for pi, ph in enumerate(phi):
                f.write(f"{fr:.12e},{ph:.12e},{e_theta[fi,pi].real:.12e},{e_theta[fi,pi].imag:.12e},{h_phi[fi,pi].real:.12e},{h_phi[fi,pi].imag:.12e},{p_theta[fi,pi]:.12e}\n")
...
def _build_material_maps(cfg: dict):
    ...
    dx = float(cfg["x_range"]) / nx
    dy = float(cfg["y_range"]) / ny
    er = np.ones((nx, ny), dtype=float)
    mrxx = np.ones((nx, ny), dtype=float)
    mryy = np.ones((nx, ny), dtype=float)
    for o in cfg.get("objects", []):
        x0, x1 = _span_to_idx(o["x_range"], dx, nx)
        y0, y1 = _span_to_idx(o["y_range"], dy, ny)
        er[x0:x1, y0:y1] = float(o.get("er", 1.0))
...
def _wg_modes_y_ez(er, mrxx, mryy, dx, c0, ix0, ix1, iy, f_center, num_modes):
    ...
    er_vec = np.asarray(er[lo:hi, iy], dtype=float)
    mrxx_vec = np.asarray(mrxx[lo:hi, iy], dtype=float)
    mryy_vec = np.asarray(mryy[lo:hi, iy], dtype=float)
    erzz_diag = spdiags(er_vec, 0, shape=(nxm, nxm))
    mrxx_inv = spdiags(1.0 / mrxx_vec, 0, shape=(nxm, nxm))
    mryy_inv = spdiags(1.0 / mryy_vec, 0, shape=(nxm, nxm))
    d_plus = np.ones(nxm)
    d_minus = -np.ones(nxm)
    ...
    for m in range(evecs.shape[1]):
        ez = evecs[:, m]
        phase = np.angle(ez[np.argmax(np.abs(ez))])
        ez = (ez * np.exp(-1j * phase)).real
        ez = ez / (np.max(np.abs(ez)) + 1e-30)
        hx = np.asarray((-(n_eff[m]) * (mrxx_inv @ ez))).squeeze().real
        ez_modes.append(ez)
        hx_modes.append(hx)
    return np.asarray(ez_modes), np.asarray(hx_modes), np.asarray(n_eff, dtype=float), lo, hi


def _wg_modes_x_ez(er, mrxx, mryy, dy, c0, iy0, iy1, ix, f_center, num_modes):
    from scipy.sparse import diags as spdiags
    from scipy.sparse.linalg import eigs
    ...
    if nym < 2:
        return None
    k0 = 2.0 * np.pi * float(f_center) / c0
    er_vec = np.asarray(er[ix, lo:hi], dtype=float)
    mrxx_vec = np.asarray(mrxx[ix, lo:hi], dtype=float)
    mryy_vec = np.asarray(mryy[ix, lo:hi], dtype=float)
    erzz_diag = spdiags(er_vec, 0, shape=(nym, nym))
    mrxx_inv = spdiags(1.0 / mrxx_vec, 0, shape=(nym, nym))
    mryy_inv = spdiags(1.0 / mryy_vec, 0, shape=(nym, nym))
    d_plus = np.ones(nym)
    d_minus = -np.ones(nym)
    ...
    for m in range(evecs.shape[1]):
        ...
        ez = (ez * np.exp(-1j * phase)).real
        ez = ez / (np.max(np.abs(ez)) + 1e-30)
        hy = np.asarray((-(n_eff[m]) * (mryy_inv @ ez))).squeeze().real
        ez_modes.append(ez)
        hy_modes.append(hy)
    return np.asarray(ez_modes), np.asarray(hy_modes), np.asarray(n_eff, dtype=float), lo, hi
...
def prepare_waveguide_profiles_from_config(cfg: dict):
    ...
    er, mrxx, mryy = _build_material_maps(out)
    ...
    for s in out.get("sources", []):
        ...
        if k == "waveguide-y":
            res = _wg_modes_y_ez(er, mrxx, mryy, dx, c0, ix0, ix1, iy0, f_center, modes_to_show)
            if res is None:
                continue
            ez_modes, hx_modes, n_eff, lo, hi = res
            mi = min(mode_index, ez_modes.shape[0] - 1)
            s["profile"] = ez_modes[mi].astype(float).tolist()
            s["profile_e"] = ez_modes[mi].astype(float).tolist()
            s["profile_h"] = hx_modes[mi].astype(float).tolist()
            s["n_eff"] = float(n_eff[mi])
            if bool(s.get("is_show", False)):
                x_axis = (np.arange(lo, hi) + 0.5) * dx
                rows = min(ez_modes.shape[0], modes_to_show)
            ...
            res = _wg_modes_x_ez(er, mrxx, mryy, dy, c0, iy0, iy1, ix0, f_center, modes_to_show)
            if res is None:
                continue
            ez_modes, hy_modes, n_eff, lo, hi = res
            mi = min(mode_index, ez_modes.shape[0] - 1)
            s["profile"] = ez_modes[mi].astype(float).tolist()
            s["profile_e"] = ez_modes[mi].astype(float).tolist()
            s["profile_h"] = hy_modes[mi].astype(float).tolist()
            s["n_eff"] = float(n_eff[mi])
            if bool(s.get("is_show", False)):
