...
class FDTD_2D_Ez:
    ...
    def calculate_source_power_fft(self, source_index=0, window='hann', detrend=True):
        ...
        if k == 'point':
            ...
            E_t = np.asarray(s.get('Ez_src', np.array([1.0])), dtype=complex)
            H_t = np.asarray(s.get('Hx_src', np.array([1.0])), dtype=complex)
            n = min(E_t.size, H_t.size)
            geometry_factor = float(np.real(np.sum(E_t[:n] * np.conj(H_t[:n]))))
        elif k == 'waveguide-x':
            E_t = np.asarray(s.get('Ez_src', np.array([1.0])), dtype=complex)
            H_t = np.asarray(s.get('Hy_src', np.array([1.0])), dtype=complex)
            n = min(E_t.size, H_t.size)
            geometry_factor = float(np.real(np.sum(E_t[:n] * np.conj(H_t[:n]))))
        elif k == 'sftf':
