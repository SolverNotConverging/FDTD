...
class FDTD_2D_Ez:
    ...
    def to_state_dict(self, dtype=np.float32, include_histories=True):
        ...
        kind_code_map = {'point': 0, 'line-soft': 1, 'sftf-x': 2, 'sftf-y': 3, 'waveguide-x': 4, 'waveguide-y': 5}
    ...
    def save_npz(self, path, dtype=np.float32, include_histories=True, retries=30, retry_wait=0.2):
        """
        Atomic, Windows-friendly save:
          - writes to <basename>.part first (no .npz extension)
          - then os.replace(...) to the final path with retries
          - ensures no '.tmp...npz' files appear
        """
        ...
        # clean up any ancient orphan parts from prior crashes (best effort)
        for old in glob.glob(os.path.join(d, base + ".part.*")):
            try:
                os.remove(old)
            except Exception:
                pass

        # create temp *.part (NO .npz!); write via file handle so NumPy won't add .npz
        fd, tmp = tempfile.mkstemp(prefix=base + ".part.", suffix="", dir=d)
        try:
            with os.fdopen(fd, "wb") as f:
                np.savez_compressed(f, **state)  # writing to handle avoids auto '.npz'

            # retry replace in case target is briefly locked (AV/sync/preview)
            for _ in range(retries):
                ...
                # could not replace after retries; leave a clear message
                raise PermissionError(
                    f"Could not replace '{path}' after {retries} retries. "
                    f"The valid data is in '{tmp}'. Close any program holding '{path}' and rename the .part file."
                )
        finally:
    ...
    def load_npz(cls, path, mmap=False):
        ...
        if "sources" in data.files:
            ...
            kind_map = {0: 'point', 1: 'line-soft', 2: 'sftf-x', 3: 'sftf-y', 4: 'waveguide-x', 5: 'waveguide-y'}
            for row in mat:
                s = dict(
        ...
        for name in ("Hx_history", "Hy_history", "Ez_history", "Dz_history"):
            if name in data.files and data[name].size:
                setattr(sim, name, data[name])
                has_hist = True

        # set history flags so show_animation works after loading from file
        sim.is_include_history = bool(has_hist)
        if has_hist and hasattr(sim, "Hx_history"):
            try:
                sim.Nt_rec = int(sim.Hx_history.shape[0])
            except Exception:
                pass
