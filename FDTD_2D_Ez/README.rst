...
to Python. ``config("gpu")`` uses Numba-CUDA when available.
``config("python")`` forces the reference loops. Loss and constitutive updates
are equivalent across all three modes.
