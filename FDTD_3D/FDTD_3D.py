...
class FDTD_3D:
    ...
    def add_material(self, name, epsilon_r=1.0, mu_r=1.0, sigma_e=0.0,
        ...
        er = as_triple(epsilon_r, "epsilon_r", positive=True)
        mr = as_triple(mu_r, "mu_r", positive=True)
    ...
    def _paint(self, material, predicate, bounds, subpixel=None):
        ...
        if mat.kind in {"PEC", "PMC"}:
            ...
            pec[occupied] = False;
            self.PEC_cells[region] = pec
            pmc = self.PMC_cells[region];
            pmc[occupied] = False;
            self.PMC_cells[region] = pmc
        self._refresh_material_state()
        return self
    ...
    def _g(self, source, t):
        ...
        if np.isclose(source["f_min"], source["f_max"]):
            ...
            return source["amplitude"] * (1 - np.exp(-(tau * max(f, 1e-30)) ** 3)) * np.sin(
                2 * np.pi * f * (t - source["t0"]))
        f = 0.5 * (source["f_min"] + source["f_max"])
        return source["amplitude"] * np.sin(2 * np.pi * f * (t - source["t0"])) * np.exp(
            -((t - source["t0"]) / source["tw"]) ** 2)
    ...
    def add_plane_monitor(self, axis, position, first=None, second=None, index=None,
        ...
        counts = (self.Nx, self.Ny, self.Nz)
        spacings = (self.dx, self.dy, self.dz)
        fixed = int(np.clip(position, 0, counts[a] - 1)) if isinstance(position, (int, np.integer)) else int(
            np.clip(np.floor(float(position) / spacings[a]), 0, counts[a] - 1))
        ...
        def parse(span, q):
            ...
            for value in span:
                vals.append(
                    int(value) if isinstance(value, (int, np.integer)) else int(np.floor(float(value) / spacings[q])))
            lo, hi = sorted(vals)
    ...
    def _compile_sources(self, steps):
        ...
        for sid, source in enumerate(self.sources):
            ...
            coords.append(c);
            ids.extend([sid] * len(c));
            pols.extend([pol] * len(c))
    ...
    def _kernel_arguments(self, source_data, monitor_coords, history, stride):
        ...
        cpml = []
        for name in "xyz": cpml.extend(
            [p[name]["node_k"], p[name]["node_b"], p[name]["node_c"], p[name]["cell_k"], p[name]["cell_b"],
             p[name]["cell_c"]])
    ...
    def run(self, steps=None, record_stride=1, reset=False, progress=True,
        ...
        for monitor in self.monitors:
            ...
            item = {**monitor, "dt": self.dt, "time": times.copy(),
                    "fields": history[:, offset:offset + count, :].reshape((len(times),) + monitor["shape"] + (6,))}
            self.monitor_results.append(item);
            offset += count
    ...
    def _update_h_numpy(self):
        ...
        curl = dzy / p["y"]["cell_k"][None, :, None] + self.Psi_Hx_y - dyz / p["z"]["cell_k"][
            None, None, :] - self.Psi_Hx_z
        ...
        curl = dxz / p["z"]["cell_k"][None, None, :] + self.Psi_Hy_z - dzx / p["x"]["cell_k"][
            :, None, None] - self.Psi_Hy_x
        ...
        curl = dyx / p["x"]["cell_k"][:, None, None] + self.Psi_Hz_x - dxy / p["y"]["cell_k"][
            None, :, None] - self.Psi_Hz_y
    ...
    def _update_e_numpy(self):
        ...
        self.Psi_Ex_y[:, 1:-1, 1:-1] = p["y"]["node_b"][None, 1:-1, None] * self.Psi_Ex_y[:, 1:-1, 1:-1] + \
                                       p["y"]["node_c"][None, 1:-1, None] * dzy[:, :, 1:-1]
        self.Psi_Ex_z[:, 1:-1, 1:-1] = p["z"]["node_b"][None, None, 1:-1] * self.Psi_Ex_z[:, 1:-1, 1:-1] + \
        ...
        self.Psi_Ey_z[1:-1, :, 1:-1] = p["z"]["node_b"][None, None, 1:-1] * self.Psi_Ey_z[1:-1, :, 1:-1] + \
                                       p["z"]["node_c"][None, None, 1:-1] * dxz[1:-1, :, :]
        self.Psi_Ey_x[1:-1, :, 1:-1] = p["x"]["node_b"][1:-1, None, None] * self.Psi_Ey_x[1:-1, :, 1:-1] + \
        ...
        self.Psi_Ez_x[1:-1, 1:-1, :] = p["x"]["node_b"][1:-1, None, None] * self.Psi_Ez_x[1:-1, 1:-1, :] + \
                                       p["x"]["node_c"][1:-1, None, None] * dyx[:, 1:-1, :]
        self.Psi_Ez_y[1:-1, 1:-1, :] = p["y"]["node_b"][None, 1:-1, None] * self.Psi_Ez_y[1:-1, 1:-1, :] + \
    ...
    def _dft(self, monitor, freqs, window=None, detrend=True):
        ...
        data = monitor["fields"].reshape(len(monitor["time"]), -1, 6)
        ...
        if window is None:
            w = np.ones(len(data))
        elif str(window).lower() in {"hann", "hanning"}:
            w = np.hanning(len(data))
        elif str(window).lower() == "hamming":
            w = np.hamming(len(data))
        elif str(window).lower() == "blackman":
    ...
    def _source_dft(self, index, freqs, window=None, detrend=True):
        ...
        f = self._frequencies(freqs);
        values = self.last_source_waveforms[:, int(index)].copy()
        ...
        if window is None:
            ...
            w = np.blackman(len(values))
        else:
            raise ValueError("window must be None, hann, hamming, or blackman.")
        kernel = np.exp(-2j * np.pi * f[:, None] * self.last_source_time[None, :]) * w[None, :] / max(float(w.sum()),
                                                                                                      1e-30)
    ...
    def power_spectrum(self, monitor_index, freqs, source_index=None, window=None, detrend=True):
        ...
        flux = 0.5 * np.einsum("fpi,fpi->fp", np.cross(fields[:, :, :3], np.conj(fields[:, :, 3:])),
                               monitor["normal"][None, None, :])
        ...
        if source_index is None:
            result["power"] = raw.real;
            result["complex_power"] = raw
        else:
            source = self._source_dft(source_index, freqs, window, detrend);
            source_power = np.abs(source) ** 2
            valid = source_power >= 1e-12 * max(float(source_power.max()), 1e-30)
            normalized = np.zeros_like(raw);
            normalized[valid] = raw[valid] / source_power[valid]
    ...
    def NF2FF(self, surfaces, freqs, theta=None, phi=None, r_obs=1.0,
        ...
        Eth = np.zeros((len(f), len(rhat)), complex);
        Eph = np.zeros_like(Eth)
        monitor_data = []
        for mid in ids:
            m = self._monitor_by_id(mid);
            monitor_data.append((m, self._dft(m, f, window, detrend)))
        for fi, freq in enumerate(f):
            ...
            Np = np.sum(N * phat, axis=1)
            Lt = np.sum(L * that, axis=1);
            Lp = np.sum(L * phat, axis=1)
        ...
        if source_index is not None:
            ...
            Eth[valid] /= spectrum[valid, None];
            Eph[valid] /= spectrum[valid, None]
            Eth[~valid] = 0;
            Eph[~valid] = 0
        shape = (len(f), len(theta), len(phi));
        Eth = Eth.reshape(shape);
        Eph = Eph.reshape(shape)
    ...
    def plot_power_spectrum(self, spectrum, db=False, ax=None):
        ...
        ax.set(xlabel="Frequency (GHz)",
               ylabel=("Power (dB)" if db else ("Source-normalized power" if spectrum["normalized"] else "Power (W)")),
               title=f"Plane power: monitor {spectrum['monitor_index']}")
    ...
    def plot_nf2ff_cut(self, result, frequency_index=0, theta_index=None,
        ...
        ax.plot(phi, values)
        ax.set_title(
            f"NF2FF cut at {result['freqs'][frequency_index] / 1e9:.4g} GHz, theta={np.degrees(theta[ti]):.1f} deg")
    ...
    def plot_plane_monitor(self, monitor, component="Ez", time_index=-1,
        ...
        if frequency is None:
            ...
            default_rep = "real";
            detail = f"t={data['time'][ti]:.6g} s"
        else:
            freq = float(frequency)
            phasor = self._dft(data, [freq], window, detrend)[0].reshape(data["shape"] + (6,))
            if key in components:
                values = phasor[..., components[key]]
            elif key == "e":
                values = np.sqrt(np.sum(np.abs(phasor[..., :3]) ** 2, axis=-1))
            elif key == "h":
                values = np.sqrt(np.sum(np.abs(phasor[..., 3:]) ** 2, axis=-1))
            else:
                values = 0.5 * np.real(
                    np.einsum("...i,i->...", np.cross(phasor[..., :3], np.conj(phasor[..., 3:])), data["normal"]))
            default_rep = "magnitude";
            detail = f"f={freq / 1e9:.6g} GHz"
        rep = default_rep if representation is None else str(representation).lower()
        if np.iscomplexobj(values):
            if rep in {"magnitude", "abs"}:
                plotted = np.abs(values); rep = "magnitude"
            elif rep == "phase":
                plotted = np.angle(values)
            elif rep == "real":
                plotted = np.real(values)
            elif rep in {"imag", "imaginary"}:
                plotted = np.imag(values); rep = "imaginary"
            else:
                raise ValueError("representation must be magnitude, phase, real, or imag.")
        else:
    ...
    def animate_plane_monitor(self, monitor, component="Ez", frame_stride=1,
        ...
        if vmin is None and vmax is None:
            if nonnegative:
                vmin = 0.0;
                vmax = max(data_max, 1e-30)
            else:
                limit = max(abs(data_min), abs(data_max), 1e-30);
                vmin = -limit;
                vmax = limit
        elif vmin is None:
            vmin = data_min
    ...
    def plot_slice(self, component="Ez", axis="z", index=None, cmap="RdBu_r", ax=None):
        ...
        ax.figure.colorbar(image, ax=ax, label=component);
        ax.set_title(f"{component}: {axis} index {index}")
        ax.figure.tight_layout();
        return ax.figure, ax
