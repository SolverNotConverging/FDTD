...
from FDTD_2D_Hz.FDTD_2D_Hz import FDTD_2D_Hz


class FDTD_2D_Hz_CPML(FDTD_2D_Hz):
    """TEz FDTD solver with convolutional PML boundaries."""
    ...
    def add_CPML(self, pml_width, sigma_max, kappa_max=5.0, alpha=0.05, order=3, direction='xy'):
        ...
        def _build_profile(N, npml, scale_const):
            sigma = np.zeros(N)
            kappa = np.ones(N)
        ...
        if dir_x:
            sig_x_e, kap_x_e, alp_x_e, b_x_e, c_x_e = _build_profile(self.Nx, npx, self.eps0)
            sig_x_h, kap_x_h, alp_x_h, b_x_h, c_x_h = _build_profile(self.Nx, npx, self.mu0)
        else:
        ...
        if dir_y:
            sig_y_e, kap_y_e, alp_y_e, b_y_e, c_y_e = _build_profile(self.Ny, npy, self.eps0)
            sig_y_h, kap_y_h, alp_y_h, b_y_h, c_y_h = _build_profile(self.Ny, npy, self.mu0)
        else:
        ...
        self.sigma_ex.fill(0.0)
        self.sigma_ey.fill(0.0)
        self.sigma_bx.fill(0.0)
        self.sigma_by.fill(0.0)
    ...
    def calculate_CE(self):
        ...
        for nx in range(self.Nx):
            for ny in range(self.Ny - 1):
                diff = (self.Hz[nx, ny + 1] - self.Hz[nx, ny]) / self.dy
                psi = self._psi_CEx_y[nx, ny]
                psi = b_y[ny] * psi + c_y[ny] * diff
                self._psi_CEx_y[nx, ny] = psi
                self.CEx[nx, ny] = (diff / kappa_y[ny]) + psi
            if per_y:
                diff = (self.Hz[nx, 0] - self.Hz[nx, self.Ny - 1]) / self.dy
            else:
                diff = (0.0 - self.Hz[nx, self.Ny - 1]) / self.dy
        ...
        for ny in range(self.Ny):
            for nx in range(self.Nx - 1):
                diff = (self.Hz[nx + 1, ny] - self.Hz[nx, ny]) / self.dx
                psi = self._psi_CEy_x[nx, ny]
                psi = b_x[nx] * psi + c_x[nx] * diff
                self._psi_CEy_x[nx, ny] = psi
                self.CEy[nx, ny] = -(diff / kappa_x[nx] + psi)
            if per_x:
                diff = (self.Hz[0, ny] - self.Hz[self.Nx - 1, ny]) / self.dx
            else:
                diff = (0.0 - self.Hz[self.Nx - 1, ny]) / self.dx
    ...
    def calculate_CH(self):
        ...
        for nx in range(self.Nx):
            for ny in range(self.Ny):
                if nx == 0:
                    if per_x:
                        diff_x = (self.Ey[nx, ny] - self.Ey[self.Nx - 1, ny]) / self.dx
                    else:
                        diff_x = (self.Ey[nx, ny] - 0.0) / self.dx
                else:
                    diff_x = (self.Ey[nx, ny] - self.Ey[nx - 1, ny]) / self.dx

                if ny == 0:
                    if per_y:
                        diff_y = (self.Ex[nx, ny] - self.Ex[nx, self.Ny - 1]) / self.dy
                    else:
                        diff_y = (self.Ex[nx, ny] - 0.0) / self.dy
                else:
                    diff_y = (self.Ex[nx, ny] - self.Ex[nx, ny - 1]) / self.dy
